...
{
	const auto MonsterCharacter = Cast<AMonsterCharacterBase>(OwnerComp.GetAIOwner()->GetPawn());
	if (MonsterCharacter == nullptr)
		return EBTNodeResult::Failed;

	const auto MonsterAnim = Cast<UMonsterBaseAnimInstance>(MonsterCharacter->GetMesh()->GetAnimInstance());
	if (MonsterAnim->Montage_IsPlaying(MonsterAnim->MonsterDeathMontage))
		return EBTNodeResult::Failed;
...
void UBTTask_MonsterAttack::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	if (!MonsterCharacter->GetIsAttacking()) // UAISkillBase::bMonsterAnimationRunning로 대신하면 보스가 평타 공격을 실행할 때 켜진 플래그가 끝날 때까지 안꺼져서 태스크랑 처음 실행할 때를 포함하여 (바로 InProgress로 넘어가니) 영원히 Succeeded를 반환하지 못하게된다.
	{
